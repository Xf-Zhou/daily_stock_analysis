...
> 说明：Web「候选」页第一版不新增后端候选池 API，也不接入 LLM 推荐。页面先以 `stocks.index.json` 的市场、行业、关键词筛选结果作为候选全集，再按「综合关注 / 趋势活跃 / 成交活跃 / 回调观察」模式串行读取必要的 `GET /api/v1/stocks/rankings` 榜单信号；仅当主榜单返回 `unavailable` / `unsupported` 且没有任何条目时，才会停止后续榜单请求，避免冷缓存或未支持市场放大外部行情源请求。榜单只给静态候选内已有股票叠加价格、涨跌幅、成交额/成交量和事实型理由，不会把不匹配关键词或筛选条件的榜单股票扩张进候选列表。候选页状态会区分实时、部分行情、缓存行情和静态候选；所有榜单不可用时仍可按静态热度展示候选。关注分仅表示“值得进一步查看的信号强度”，不是买入评级或投资建议；只有用户主动点击星标后，股票才会写入 `STOCK_LIST` 并进入后续定时分析与通知流程。
